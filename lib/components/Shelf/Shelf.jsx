...
class Shelf extends PureComponent {
  ...
  componentDidUpdate () {
    ...
    if (isLoaded && rootElement.classList.contains('shelf--loaded') !== true) {
      requestAnimationFrame(() => {
        rootElement.classList.add('shelf--loaded')
      })
    }
  }
  ...
  renderHeroImage () {
    ...
    if (
      ...
      return (
        <HeroImage
          className={['shelf__background-hero']}
          hasOverlay
  ...
  renderIconClose () {
    ...
    if (state === ANIMATION_STATES.FINISHED) {
      return (
        <Icon
          iconClass={['icon--close', 'icon--white', 'shelf__close-icon']}
