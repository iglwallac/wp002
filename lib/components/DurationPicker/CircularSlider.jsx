...
// This is a library file but required modifications to work in our stack.
...
export default class CircularSlider {
    ...
    /**
     * Sets slider value based on the provided step number. We
     * don't allow setting the value directly since
     ...
     * Animates slider by setting stroke-dashoffset and transform properties in
    ...
    [_setStepFallback] (stepNo) {
      ...
      const animate = () => {
        ...
        radiansMove += (Math.abs(radiansEnd - radiansMove) <= 0.2) ?
          (isIncreasing ? 0.01 : -0.01) : (isIncreasing ? 0.05 : -0.05)
    ...
    [_canMove] (newPosition) {
      return !(this.position.y < 0 && ((this.position.x >= 0 && newPosition.x < 0)
      || (this.position.x < 0 && newPosition.x >= 0)))
    }

    /**
     * Calculates new position, angles and path traveled based on local coordinate
     * system (center = 0,0).
     *
     * @param angleRadians
    ...
    /**
     * Creates slider composed of underlying stripped SVG circle and top
     * colored circle which will behave as slider.
     */
    [_initSlider] () {
    ...
    [_step2Rad] (stepNo) {
      ...
      const range = this.options.max - this.options.min
      const degrees = this.options.max === val ? 359.99 :
        (Math.round(adjustedVal * (360.0 / range))) % 360
