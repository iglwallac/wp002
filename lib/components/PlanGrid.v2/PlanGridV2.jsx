...
/**
 * Function for determining the default plan to show when the user initially sees the plan grid
 * @param {Boolean} isLoggedIn Whether or not the user is logged in
 * @param {Boolean} allowAuthenticatedFreeTrialSkus Whether or not the user is eligible
 * to receive the trial offer skus
 * @param {String} selectedPlanId The ID of the selected plan
 * @returns {Object} The default state
 */
function getDefaultState (isLoggedIn, allowAuthenticatedFreeTrialSkus, chosenPlanType) {
  let sku = PLAN_SKU_ONE_WEEK_FREE_TRIAL_TO_ANNUAL
  let type = PLAN_SUBSCRIPTION_ANNUAL

  if (isLoggedIn && !allowAuthenticatedFreeTrialSkus) {
    sku = PLAN_SKU_ANNUAL
  }

  if (chosenPlanType && chosenPlanType !== PLAN_SUBSCRIPTION_UNKNOWN) {
    type = chosenPlanType
  }
...
function onPlanSelected (type, sku, selectedType, selectedSku, update) {
  if (type !== selectedType || sku !== selectedSku) {
    update({ type, sku })
  }
}
...
function PlanGridV2 (props) {
  ...
  // update the selected plan on login ect.
  useEffect(() => {
    updatePlanSelected(getDefaultState(
      isLoggedIn,
      allowAuthenticatedFreeTrialSkus,
      chosenPlanType,
    ))
  }, [isLoggedIn, allowAuthenticatedFreeTrialSkus])
        ...
        <div className={`plan-grid-v2__button-wrapper${isCheckoutChangePlan || isAccountChangePlan ? ' plan-grid-v2__button-wrapper--multiple' : ''}`}>
          {
            !isCheckoutChangePlan && !isAccountChangePlan ?
              <Button
                ...
                onClick={() => onChoosePlanClick(selectedPlanDetails, currencyIso, props)}
              >
                {getCtaText(props, allowAuthenticatedFreeTrialSkus, planSelected.type)}
              </Button>
              : null
          }
          {
